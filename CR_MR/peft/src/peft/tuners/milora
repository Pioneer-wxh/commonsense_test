...
class Linear(nn.Linear, LoraLayer):
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, "lora_A"):
            ...
            # 选取最小的r个奇异值及对应的奇异向量 (small mode)
            u_sub = u[:, -self.r:].detach().clone().contiguous().to(self.weight.device)
            s_sub = s[-self.r:].detach().clone().contiguous().to(self.weight.device)
            vt_sub = vt[-self.r:, :].detach().clone().contiguous().to(self.weight.device)

            # Initialize adapter parameters
            s_sub /= self.scaling
            S_sqrt = torch.sqrt(s_sub).to(adapter_dtype)
            S_diag = torch.diag(S_sqrt).to(adapter_dtype)
            
            # 确保维度匹配
            self.lora_A.weight.data = (S_diag @ u_sub.T).contiguous().to(adapter_dtype)  # (r, r) @ (r, in_features) = (r, in_features)
            self.lora_B.weight.data = (vt_sub.T @ S_diag).contiguous().to(adapter_dtype)  # (out_features, r) @ (r, r) = (out_features, r)
            
            # 合并适配器
            merge = self.lora_B.weight @ self.lora_A.weight  # (out_features, r) @ (r, in_features) = (out_features, in_features)
            self.weight.data = (self.weight.data - merge * self.scaling).to(weight_dtype)
