...
2. 需要特别注意的是，如果使用sorsa算法，则需要先复制粘贴sorsa.py到lora.py，然后再确定一下peft仓库中两个__init__.py文件中是否正确引用了sorsa_trainer中的Trainer和Trainargument（这两个类单独放在了trainer.py文件中，因为sora最终要复制到lora中，所以这些类都含有LoRA前缀，从而保证一致。后期如果再有这样需要额外的参数来进行训练的，可以仿照类似的结构进行修改）
## 修改mapping
当使用新的模型时，要修改此文件，具体如何修改，可以参考注释`#修改`，
本仓库主要是修改添加了Qwen模型
