...
from peft import (  # noqa: E402
    ...
    LORATrainer, #使用sorsa修改为此
    LORATrainingArguments,#使用sorsa修改为此
)
...
def train(
        ...
        save_step: int = 200,
        # lora hyperparams
        gamma: float = 4e-4,#新增使用sorsa修改为此
        lora_r: int = 8,
        lora_alpha: int = 16,
    ...
    trainer = LORATrainer(#使用sorsa修改为此
        model=model,
        train_dataset=train_data,
        eval_dataset=val_data,
        args=LORATrainingArguments(#使用sorsa修改为此
            gamma=gamma,#使用sorsa修改为此
            ...
            save_total_limit=3,
            load_best_model_at_end=False,#使用sorsa修改为此
